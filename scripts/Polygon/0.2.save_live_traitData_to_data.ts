...
async function main() {
    ...
    for(let i = 0; i < LiveTraitLength; i++) {
        const TraitData = await LiveECRegistry.getData(i, 0, 1250);
        traitData.push(TraitData);
        console.log(i, "saved");
    }

    try {
        fs.writeFileSync('./scripts/Polygon/data/liveTraitData.json', JSON.stringify(traitData));
